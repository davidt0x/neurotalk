...
from io import open
import random, datetime, argparse, socket, time, struct, subprocess, re, csv, multiprocessing, pyaudio
import pandas as pd
...
def magicNumbers():
    ...
    dyad_str = str(DYAD)
    subj_str = str(SUBJ)
    savefileLog = _thisDir + os.sep + u'data/%s_%s-%s_TimingsLog_%s.csv' % (exp_name, dyad_str, subj_str, date_str)
    savefileTTL = _thisDir + os.sep + u'/data/%s_%s-%s_TTLtimestamps_%s.csv' % (exp_name, dyad_str, subj_str, date_str)
    savefileTimestamps = _thisDir + os.sep + u'data/%s_%s-%s_timestamps_%s.csv' % (exp_name, str(DYAD), str(SUBJ), date_str)

    return(punchTimeout, CHANNELS, RATE, FORMAT, date_str, savefileLog, savefileTTL, savefileTimestamps,
            portIn, portOut, PortIn, PortOut, PortComm, role, IP, scanner,
            exp_name, win_x, win_y, win_fullscr, start_lag, letter_h, wrap_w,
            intro_time, comm_time, between_time, key_list)
...
def outputProcess(BUFFER, CHUNK, FORMAT, CHANNELS, RATE, queueOutput, savefileTimestamps):
    ...
    while True:
        ...
        if note == 'run_end':
            ...
            messagesOutput(packetCounter, timeListServer, timeListClient,
                           packetListClient, overFlowFlag, savefileTimestamps)
...
def messagesOutput(packetCounter, timeListServer, timeListClient, packetListClient, overFlowFlag, savefileTimestamps):
    ...
    try:
        ...
        output = open(savefileTimestamps, 'a+')
...
def goGo(NAT, STUN, LOGTTL, DYAD, SUBJ, COND):
    ...
    [punchTimeout, CHANNELS, RATE, FORMAT, date_str, savefileLog, savefileTTL, savefileTimestamps,
            portIn, portOut, PortIn, PortOut, PortComm, role, IP, scanner,
            exp_name, win_x, win_y, win_fullscr, start_lag, letter_h, wrap_w,
            intro_time, comm_time, between_time, key_list] = magicNumbers()

    # networkInit
    networkInit(STUN, NAT, portIn, portOut, punchTimeout)

    # set filenames
    savefileOut = _thisDir + os.sep + u'data/DIAD_' + str(DYAD) + '-' + str(SUBJ) + '_RecordedAudio_' + date_str
    ...
    filename = _thisDir + os.sep + u'data/%s_%s-%s_%s' % (exp_name, DYAD, SUBJ, date_str)

    # An ExperimentHandler isn't essential but helps with data saving
    thisExp = data.ExperimentHandler(name=exp_name, version='',
        extraInfo='', runtimeInfo=None,
        originPath= '_thisDir' + os.sep + u'DIAD_scan_air.py',
    ...
    try:
        win = visual.Window(size=[win_x, win_y], color='black', fullscr=win_fullscr, screen=1, monitor=my_monitor)
    ...
    show_sync = visual.TextStim(win, name='show_sync', text='Syncing start time with your partner...',
        pos=(0, 0), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    show_instructions = visual.TextStim(win,name='show_instructions', text='',
        pos=(0, 0), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    show_prompt = visual.TextStim(win=win, name = 'show_prompt', text = '',
        pos=(0, 0.2), height=letter_h, color='white', autoLog=False)
    show_role = visual.TextStim(win=win, name = 'show_role', text = '',
        pos=(0, 0.7), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    show_pass = visual.TextStim(win=win, name = 'show_pass', text = '',
        pos=(0, 0), height=letter_h, color='white', autoLog=False)
    show_lines = visual.TextStim(win=win, name = 'show_lines', text = '',
        pos=(0, 0), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    show_lines_done = visual.TextStim(win=win, name = 'show_lines_done', text = 'Please wait until the timer is done.',
        pos=(0, 0), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    show_timer = visual.TextStim(win=win, name = 'show_timer', text = '',
        pos=(0, -0.7), height=letter_h, color='white', autoLog=False)
    show_blank = visual.TextStim(win,name='show_blank', text='+',
        pos=(0, 0), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    show_end = visual.TextStim(win=win, name = 'show_end', text = 'You are now done with the conversation.',
        pos=(0, 0), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    ...
    persuade_instr_text = ('Next, you will discuss with the other participant how the charity money '
                       'should be allocated.'
    ...
    compromise_instr_text = ('Next, you will discuss with the other participant how the charity money '
                       'should be allocated.'
    ...
    fLog.write(',,' + str(startTimeCommon) + ',' + str(conv_clock.getTime()) + ',,' + str(fOut.tell()) + '\n')
    ...
    while blank_clock.getTime() <  between_time:
        ...
        if keys:
            ...
            if keys[0] == '5':
                ...
                fTTL.write(',,blank,' + str(time.time()) + ',' + str(conv_clock.getTime()) +',\n')
            # escape quits
            elif keys[0] == 'escape':
                cleanup(queueInput, queueOutput, audioInput,
                            audioOutput, fLog, fTTL)
    ...
    if ((first_speaker[0] == 'A') & (role == 'A')) or ((first_speaker[0] == 'B') & (role == 'B')):
        turn_role = 'speaker'
    ...
    # hdr: fLog.write('condition,role,time.time,run.time,comm.time,audio_position\n')
    fLog.write(str(COND) + ',turn_intro,' + str(time.time()) + ',' + str(conv_clock.getTime()) + ',,'
            + str(fOut.tell()) + '\n')
    ...
    while intro_clock.getTime() < intro_time:
        ...
        if keys:
            ...
            if keys[0] == '5':
                # hdr: fTTL.write('condition,role,segment,time.time,run.time,comm.time\n')
                fTTL.write(str(COND) + ',,' + 'intro' + ',' + str(time.time())+ ',' + str(conv_clock.getTime()) + ',\n')
            # escape quits
            elif keys[0] == 'escape':
                cleanup(queueInput, queueOutput, audioInput,
                             audioOutput, fLog, fTTL)
    ...
    # hdr: fLog.write('condition,role,time.time,run.time,comm.time,audio_position\n')
    fLog.write(str(COND) + ',' + turn_role + ',' + str(time.time()) + ',' + 
            str(conv_clock.getTime()) + ',' + str(comm_clock.getTime()) + ',' + str(fOut.tell()) + '\n')
    ...
    while comm_clock.getTime() < comm_time:
        ...
        if sum(mouseclick) > 0:
            ...
            if turn_role == 'speaker':
                ...
                # log audio file pointer positions at turn changes
                # hdr: fLog.write('condition,role,time.time,run.time,comm.time,audio_position\n')
                fLog.write(str(COND) + ',' + turn_role + ',' + str(timeHere) + ',' + str(runHere) + ',' +
                    str(commHere) + ',' + str(fOut.tell()) + '\n')
                ...
                # send packets
                try:
                    timestamps_to_send = struct.pack('<ddd', timeHere, runHere, commHere)
                    socketComm.sendto(timestamps_to_send, (IP, PortComm))
                    print('\nTime when the mouseclick was sent out: ' + str(datetime.datetime.fromtimestamp(time.time())))
                except:
        ...
        if keys:
            if keys[0] == '5':
                # hdr: fTTL.write('condition,role,segment,time.time,run.time,comm.time\n')
                fTTL.write(str(COND) + ',' + str(turn_role) + ',' + 'communication' + ',' + str(time.time())+ ',' +
                    str(conv_clock.getTime()) + ',' + str(comm_clock.getTime()) + '\n')
        ...
        if turn_role == 'listener':
            ...
            if bool(incoming) and bool(len(incoming) == 24):
                ...
                print('\nReceived mouseclick at time ', str(datetime.datetime.fromtimestamp(time.time())))
                turn_role = 'speaker'
                turn_role_text = turn_speak_text
                turn_pass_mic_text = 'When you want to pass the mic, click your mouse'
                show_role.setText(turn_role_text, log=True)
                ...
                # log audio file pointer positions at turn changes
                # hdr: fLog.write('condition,role,time.time,run.time,comm.time,audio_position\n')
                fLog.write(str(COND) + ',' + turn_role + ',' + str(timeThere) + ',' + str(runThere) + ',' +
                    str(commThere) + ',' + str(fOut.tell()) + '\n')
        ...
        # update screen
        show_timer.setText(str(round(comm_time - comm_clock.getTime())) + ' seconds', log=False)
    ...
    # hdr: fLog.write('condition,role,time.time,run.time,comm.time,audio_position\n')
    fLog.write(str(COND) + ',trial_end,' + str(time.time()) + ',' + str(conv_clock.getTime()) + ',' + 
        str(comm_clock.getTime()) + ',' + str(fOut.tell()) + '\n')
...
if __name__ == '__main__':
    ...
    if 0 <= args.COND <= 1:
        ...
        raise ValueError('Unexpected value for argument COND ',
                         '(should be 0 or 1)')
