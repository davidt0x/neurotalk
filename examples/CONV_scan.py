...
def magicNumbers():
    ...
    pair = PARTICIPANT[1:] # set seed to last 2 digits of SUBJect ID [should be the same for both participants]
    ...
    savefileTTL = _thisDir + os.sep + u'/data/%s_%s_TTLtimestamps_%s.csv' % (exp_name, PARTICIPANT, date_str)
    savefileTimestamps = _thisDir + os.sep + u'data/%s_%s_timestamps_%s.csv' % (exp_name, PARTICIPANT, date_str)

    return(punchTimeout, CHANNELS, RATE, FORMAT, date_str, savefileLog, savefileTTL, savefileTimestamps,
            portIn, portOut, PortIn, PortOut, PortComm, pair, role, seed_num, IP, scanner,
            exp_name, win_x, win_y, win_fullscr, stimuli_file, read_file, start_lag, items_n, runs_n, letter_h, wrap_w,
            intro_time, comm_time, between_time, key_list_speak)
...
def goGo(NAT, STUN, LOGTTL, PARTICIPANT, RUN):
    ...
    [punchTimeout, CHANNELS, RATE, FORMAT, date_str, savefileLog, savefileTTL, savefileTimestamps,
            ...
            intro_time, comm_time, between_time, key_list_speak] = magicNumbers()
    ...
    fLog.write('run,trial,item,condition,role,time.time,run.time,comm.time,audio_position\n')
    ...
    thisExp = data.ExperimentHandler(name=exp_name, version='',
        ...
        originPath= '_thisDir' + os.sep + u'CONV_scan.py',
    ...
    prompts_cond_order = [random.sample(['G','R'], 2) for prompts_cond_order in range(10)]
    ...
    show_blank = visual.TextStim(win,name='show_blank', text='+',
        pos=(0, 0), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    show_end = visual.TextStim(win=win, name = 'show_end', text = 'You are now done with this task.',
        pos=(0, 0), height=letter_h, wrapWidth=wrap_w, color='white', autoLog=False)
    ...
    for run_n in range(RUN, runs_n + 1):
        ...
        prompts_cond_run = prompts_cond_order[num_items_per_run*run_n-num_items_per_run:num_items_per_run*run_n]
        ...
        for idx in range(num_items_per_run):
            ...
            stim_list.append({'run': run_n, 'set': sets_run[idx], 'index': idx,
            'trial': idx + 1, 'item': prompts_item_run[idx],
            'question': prompts_text_run[idx], 'condition': prompts_cond_run[idx],
        ...
        instr_text = ('Task: We are interested in understanding how people converse about different topics.'
                       ...
                       '\n\nYou will have ' + '{0:.0f}'.format(comm_time/60) + ' minutes for each topic. '
        ...
        fTTL.write(str(run_n) + ',,,,,,' + str(fttl_time)+',' + str(run_clock.getTime()) + ',\n')
        ...
        fLog.write(str(run_n) + ',,,,,' + str(startTimeCommon) + ',' + str(run_clock.getTime()) + ',,' + str(fOut.tell()) + '\n')
        ...
        while blank_clock.getTime() <  between_time:
            ...
            if keys:
                ...
                if keys[0] == 'equal':
                    ...
                    fTTL.write(str(run_n) + ',,,,,blank,' + str(time.time()) + ',' + str(run_clock.getTime()) +',\n')
                # escape quits
                elif keys[0] == 'escape':
                    cleanup(queueInput, queueOutput, audioInput,
                               audioOutput, fLog, fTTL)
        ...
        for trial_i, this_trial in enumerate(trials):
            ...
            # hdr: fLog.write('run,trial,item,condition,role,time.time,run.time,comm.time,audio_position\n')
            fLog.write(str(run_n) + ',' + str(this_trial['trial']) + ',' +
                str(this_trial['item']) + ',' + str(this_trial['condition']) +
                ',trial_intro,' + str(time.time()) + ',' + str(run_clock.getTime()) + ',,'
                + str(fOut.tell()) + '\n')
            ...
            while intro_clock.getTime() < intro_time:
                ...
                if keys:
                    ...
                    if keys[0] == 'equal':
                        # hdr: fTTL.write('run,trial,item,condition,role,segment,time.time,run.time,comm.time\n')
                        fTTL.write(str(run_n) + ',' + str(this_trial['trial']) + ',' +
                            str(this_trial['item']) + ',' + str(this_trial['condition']) +
                            ',,' + 'intro' + ',' + str(time.time())+ ',' + str(run_clock.getTime()) + ',\n')
                    # escape quits
                    elif keys[0] == 'escape':
                        cleanup(queueInput, queueOutput, audioInput,
                                   audioOutput, fLog, fTTL)
            ...
            if this_trial['condition'] == 'R':
                ...
                for turn_i, turn in enumerate(trial_text):
                    ...
                    fLog.write(str(run_n) + ',' + str(this_trial['trial']) + ',' +
                        str(this_trial['item']) + ',' + str(this_trial['condition']) + ',' +
                        turn_role + ',' + str(time.time()) + ',' + str(run_clock.getTime()) + ',' + str(comm_clock.getTime()) + ',' + str(fOut.tell()) + '\n')
                    ...
                    while comm_clock.getTime() < comm_time:
                        ...
                        if keys:
                            if keys[0] == '1':
                                ...
                                if turn_role == 'speaker':
                                    ...
                                    try:
                                        ...
                                        print('\nTime when the keypress was sent out: ' + str(datetime.datetime.fromtimestamp(time.time())))
                                        break
                                    except:
                            ...
                            # if event.getKeys returns a '=', its a TTL
                                ...
                                fTTL.write(str(run_n) + ',' + str(this_trial['trial']) + ',' +
                                    str(this_trial['item']) + ',' + str(this_trial['condition']) + ',' +
                                    str(turn_role) + ',' + 'communication' + ',' + str(time.time())+ ',' +
                                    str(run_clock.getTime()) + ',' + str(comm_clock.getTime()) + '\n')
                            # escape quits
                            elif keys[0] == 'escape':
                                # transmit turn info to other computer
                                # send packets
                ...
                while comm_clock.getTime() < comm_time:
                    ...
                    if keys:
                        if keys[0] == '1':
                            ...
                            if turn_role == 'speaker':
                                ...
                                fLog.write(str(run_n) + ',' + str(this_trial['trial']) + ',' +
                                    str(this_trial['item']) + ',' + str(this_trial['condition']) + ',' +
                                    turn_role + ',' + str(timeHere) + ',' + str(runHere) + ',' +
                    ...
                    if turn_role == 'listener':
                        ...
                        if bool(incoming) and bool(len(incoming) == 24):
                            ...
                            fLog.write(str(run_n) + ',' + str(this_trial['trial']) + ',' +
                                str(this_trial['item']) + ',' + str(this_trial['condition']) +
                                ',' + turn_role + ',' + str(timeThere) + ',' + str(runThere) + ',' +
            ...
            fLog.write(str(run_n) + ',' + str(this_trial['trial']) + ',' +
                str(this_trial['item']) + ',' + str(this_trial['condition']) +
                ',trial_end,' + str(time.time()) + ',' + str(run_clock.getTime()) + ',' + str(comm_clock.getTime()) + ',' + str(fOut.tell()) + '\n')
