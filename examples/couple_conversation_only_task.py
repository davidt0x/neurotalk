    ...
    while comm_clock.getTime() < COMM_S:
        ...
        if keys:
            ...
            if key == KEY_QUIT:
                ...
                # Main CSV press
                log_comm_press(thisExp, event_name='pass_press', role_label=toggled_role, run_clock=run_clock, phase_clock=comm_clock,
                               dyad=dyad, session=session, exp_condition=exp_condition, conflict_text=conflict_text,
                               first_speaker=first_speaker, participant_role=role)
